    ...
    <div class="container p-0">
        <div class="vcard-twelve main-content  w-100 mx-auto content-blur allSection collapse show">
            <div class="vcard-one__product py-3 mt-0">
                <div class="d-flex align-items-center gap-3 justify-content-between mb-4 px-30" @if (getLanguage($vcard->default_language) == 'Arabic') dir="rtl" @endif>
                        <div class="text-start @if (getLanguage($vcard->default_language) == 'Arabic') text-end @endif">
                            <h2 class="text-white fs-24 mb-0 d-inline-block">
                                {{ __('messages.plan.products') }}
                            </h2>
                        </div>
                    <div>
                        <a class="btn btn-primary text-decoration-none" href="{{ $vcardUrl }}" role="button">{{
                            ...
                            <a @if ($product->product_url) href="{{ $product->product_url }}" @endif
                                ...
                                <div class="product-card h-100 w-100">
                                    ...
                                    <div class="product-details mt-3">
                                        <h4 class="text-white fs-20 fw-5">{{ $product->name }}</h4>
                                        <p class="mb-2 fs-14 text-gray-100">
                                            {{ $product->description }}
                                        </p>
                                        @if ($product->currency_id && $product->price)
                                        <span
                                            class="product-amount text-primary fw-6 fs-20 text-align-left product-price-{{ $product->id }}">{{
                                        ...
                                        <span
                                            class="product-amount text-primary fw-6 fs-20 product-price-{{ $product->id }}">{{
                                            getUserCurrencyIcon($vcard->user->id) }}{{ $product->price }}</span>
                                        @endif
                            </a>
                            @if (!empty($product->price))
                            <div class="mt-3 text-center">
                                <button class="product-btn btn-primary text-decoration-none buy-product"
